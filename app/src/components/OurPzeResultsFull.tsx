import { usePzeUserResults } from "../queries/usePzeUserResults";
...
export default function OurPzeResultsFull({ year, round }: Props) {  
  const {
    data: results,
    isLoading,
    isError,
    error,
  } = usePzeUserResults(year, round);

  if (isNaN(year) || year < 2026) return <div>Choose a year to see results</div>;
          ...
          {round === 1
            ...
            : round === 2
              ...
              : round === 3
                ? " Grand Final"
                : ""}
          {` Results for Pesma za Evroviziju ${year}`}
        </h2>
      </em>
