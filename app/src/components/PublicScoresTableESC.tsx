...
export default function PublicScoresTableESC({
  user_id,
  username,
  year,
  round,
  ...
  const status = useEscScoringStatus(user_id, year, round);
  const hasStarted = status.data === true;
  const contest = "esc";

  const scores = useEscUserScores(user_id, year, round, hasStarted);
      ...
      <div className={`${styles.comments_panel} scrollbarPink`}>
        ...
        {activeScore === null ? (
          ...
          <>
            <h4>
              <img
                src={activeScoreData?.flag_url}
                alt="flag"
                className={styles.flag}
              />
              <span>{activeScoreData?.country}</span>
            </h4>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {scores.data!.map((s) => (
                ...
                >
                  <td>
                    <img src={s.flag_url} alt="flag" className={styles.flag} />
                    {s.country}
                    <em>
                      <span className={styles.esc_entry}>
