...
export default function UserScoresTableESC({ year, round }: Props) {
  ...
  const scores = useEscUserScores(user!.id, year, round, hasStarted);
        ...
        <div className={`${commentStyles.comments_panel} scrollbarPink`}>
          ...
          {activeCommentsId === null ? (
            ...
            <>
              <h4>
                <img
                  src={activeCommentsData?.flag_url}
                  alt="flag"
                  className={commentStyles.flag}
                />
                <span>{activeCommentsData?.country}</span>
              </h4>
              <Comments
